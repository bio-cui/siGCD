#!/usr/bin/env Rscript
drug_gene <- function(drugname,drugtar=c(),partg=c(),expfile,clifile,confoundf=c()) {
#################################################################################################################
#    *** make sure that the drugbank.txt file is in the work directory
#    input:  drugname: drug name
#            drugtar: targets of drug ("none" if use targets of drug in DrugBank database)
...
################################################################################################################
	...
	idx=which(as.character(drugbank[,1])==drugname)
